...
MainWindow::MainWindow(/*Qt3DCore::QEntity *rootEntity, */QWidget *parent) :
    ...
    _isShowMag(false),
    _dialogCalibrate(new DialogCalibrate(this))

{
    ...
    _listIndex = 0;
    QString fileName("/home/hober/Desktop/"
                     "build-Platform_XYZ_UI-"
                     "Desktop_Qt_5_11_1_GCC_64bit-Debug/"
                     "log/Motor_log ");
...
//    _manager = new Manager3D(rootEntity);
    ...
    _dialogCalibrate->setEnvelopeAndTracker(&_en,&_tracker);
//    double data[][3] = {
...
bool MainWindow::setup(ros::NodeHandle &node)
{
    _tfReceiver = new TurtleTfReceiver;
    if(_tfReceiver->setup(node))
    {
        return true;
    };
    return false;
...
MainWindow::~MainWindow()
{
    delete ui;
    delete _serialMotor;
    delete _serialSensor;
    delete _file;
    delete _tfReceiver;
    delete _dialogCalibrate;
}
...
void MainWindow::on_actionConnect_to_NUCLEO_triggered()
{
    ...
    bool isMotorPass = true, isSensorPass = true;
    if(ui->checkBoxMotor->isChecked())
        isMotorPass = _serialMotor->open(QIODevice::ReadWrite);
    if(ui->checkBoxSensor->isChecked())
        isSensorPass = _serialSensor->open(QIODevice::ReadWrite);
    if ( isMotorPass && isSensorPass) {
        //        console->setEnabled(true);
        //        console->setLocalEchoEnabled(p.localEchoEnabled);
        ui->actionConnect_to_NUCLEO->setEnabled(false);
        ui->actionDisconnect->setEnabled(true);
        //        ui->actionConfigure->setEnabled(false);
        ui->statusBar->showMessage(tr("Sensor Connected to %1 : "
                                      "%2, %3, %4, %5, %6; "
                                      "Motor Connect to %7 : "
                                      "%8, %9, %10, %11, %12 ")
                                   .arg(_serialSensor->portName())
                                   .arg(_serialSensor->baudRate())
                                   .arg(QSerialPort::Data8)
                                   .arg(QSerialPort::NoParity)
                                   .arg(QSerialPort::OneStop)
                                   .arg(QSerialPort::NoFlowControl)
                                   .arg(_serialMotor->portName())
                                   .arg(_serialMotor->baudRate())
                                   .arg(QSerialPort::Data8)
                                   .arg(QSerialPort::NoParity)
                                   .arg(QSerialPort::OneStop)
                                   .arg(QSerialPort::NoFlowControl));
...
void MainWindow::on_actionDisconnect_triggered()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(
                this, "save pos and dir...","",tr("Text files. (*.txt)"));
    if(fileName == "") return;
...
//    mutex.lock();
    QByteArray data;
    if(_serialMotor->isOpen()) data = _serialMotor->readAll();
...
//                    ui->statusBar->showMessage("data...");
                    if(_estimater.estimate())
                    {
                        ...
                        VectorXd eigenValues = _estimater.getEigenValues();
                        MatrixXd eigenVectors = _estimater.getEigenVectors();
                        _positionHistory.push_back(pos);
                        _directionHistory.push_back(dir);
                        QString pd = tr("pos = (%1, %2, %3), dir = (%4, %5, %6)")
                                .arg(pos[0]).arg(pos[1]).arg(pos[2])
                                .arg(dir[0]).arg(dir[1]).arg(dir[2]);
                        std::stringstream eigenStream;
                        eigenStream << "eigenValues:\n" << eigenValues << "\n";
                        eigenStream << "eigenVectors:\n" << eigenVectors;
                        eigenString = QString::fromStdString(eigenStream.str());
                        ui->statusBar->showMessage(pd);

                        double mapAngle, mapX, mapY;
                        if(_tfReceiver->spin())
                        {
                            mapAngle = _tfReceiver->angle();
                            mapX = _tfReceiver->x();
                            mapY = _tfReceiver->y();
                            eigenString += tr("\n(x, y, theta): (%1, %2, %3)")
                                    .arg(mapX).arg(mapY).arg(mapAngle);
                        }
...
//                        _manager->setDirection(dir[0],dir[1],dir[2]);
            ...
            QString data = tr("%1%2 %3 %4 %5 %6").arg(QChar(cmd)).arg(magIndex)
                    .arg(x).arg(y).arg(z).arg(current.toString());
            if( eigenString != "" ){
                data = data + "\n" + eigenString;
                eigenString = "";
            }
...
void MainWindow::sendData(const char *command, int size)
{
    ...
    if(command[0] == 'M' ||
            ...
            command[0] == 'S' ||
            command[0] == 'R')
        _serialSensor->write(dataToSend);
    else if(_serialMotor->isOpen()) _serialMotor->write(dataToSend);
    qDebug()<<dataToSend;
}

void MainWindow::finishCalibrate(bool success)
{
    connect(_serialSensor, &QSerialPort::readyRead, this, &MainWindow::readData);
    if(!success)
        ui->statusBar->showMessage("calibrate canceled");
    else
    {
        ui->statusBar->showMessage("calibrate successed");
        if(!_dialogCalibrate->isDataReady())
        {
            ui->statusBar->showMessage("calibrate failed...");
            return;
        }
        Matrix3d *cU = _dialogCalibrate->U();
        Vector3d *cc = _dialogCalibrate->c();
        double* U = new double [NUMBER_OF_SENSOR*9];
        double* c = new double [NUMBER_OF_SENSOR*3];
        for(int i = 0; i < NUMBER_OF_SENSOR; i++)
        {
            cU[i].transposeInPlace();
            for(int j = 0; j < 9; j++) U[i*9+j] = cU[i].data()[j];
            for(int j = 0; j < 3; j++) c[i*3+j] = cc[i].data()[j];
        }
        _magDataPreprocessor.setUAndC(U,c);
        _estimater.setRotMat(_dialogCalibrate->rotation());
    }
}
...
void MainWindow::on_pushButtonEcho_clicked()
{
    //sendData("O",1);
    echo_mag();
    _tfReceiver->setFrames(ui->lineEditOriginFrame->text().toStdString().c_str(),
                           ui->lineEditDestinationFrame->text().toStdString().c_str());
}
...
void MainWindow::setMagSel(MagnetometersSelection ms, bool sel)
{
    ...
    _magDataPreprocessor.setSelectedData((unsigned char *)(&_magSelection));
...
void MainWindow::on_actionOpen_triggered()
{
    QString inputFileName = QFileDialog::getOpenFileName(
                this, "open record pattern", "", tr("Pattern files (*.ptn)"));
//    QString inputFileName = "D:\\Magnet localization\\electroMag.ptn";
    ...
    if(inputFile->open(QIODevice::ReadOnly|QIODevice::Text))
    {
        ...
        _textStream.setCodec("UTF-8");
        _textStream >> _xStart >> _xEnd >> _xStep
                >> _yStart >> _yEnd >> _yStep
                >> _zStart >> _zEnd >> _zStep >> _num;
        qDebug() << _xStart << _xEnd << _xStep
                 << _yStart << _yEnd << _yStep
                 << _zStart << _zEnd << _zStep << _num;
...
{
    QString calibrationFileName = QFileDialog::getOpenFileName(
                this, "load calibration file", "", tr("Text file (*.txt *.log)"));
...
void MainWindow::on_horizontalSliderRecordTime_sliderMoved(int position)
{
...
{
    _dialogCalibrate->show();
    _dialogCalibrate->setSerial(_serialSensor);
    disconnect(_serialSensor, &QSerialPort::readyRead, this, &MainWindow::readData);
}
...
void MainWindow::on_actionAbout_triggered()
{
    ...
    aboutContent =
            "This is a program for magnetic field localization.\n"
            "The author is Hober Hsieh.\n"
            "If you have any problems, please contact hober405@gmail.com.\n";
    QMessageBox::about(this, "About Platform XYZ", aboutContent);
}
