...
void MagnetEstimator::inititialize()
{
    ...
    Matrix3d rotBase; rotBase << 0, -1, 0,
                                 1, 0, 0,
                                 0, 0, 1;
    Vector3d posBase; posBase << -1.5, 0, 2.2;
    int layerCount = 2;
    double layerGap = -2.2;
    int numberOfSensorPerLayer = _numberOfSensor/layerCount;
    rotDir.normalize();
    double rotRad = -90.0*EIGEN_PI/180.0;
    double layerRotRad = -45.0*EIGEN_PI/180.0;
    ...
    Matrix3d layerRot;
    layerRot = AngleAxisd(layerRotRad, rotDir);

    std::stringstream ss;
    int index;
    for(int layer = 0; layer < layerCount; layer++)
    {
        for(int i = 0; i < numberOfSensorPerLayer; i++)
        {
            index = layer*numberOfSensorPerLayer+i;
            if(index == 0) continue;
            _rotMat.push_back(rot*_rotMat[index-1]);
            _pos.col(index) = (rot*_pos.col(index-1));
            if(i == 0)
            {
                _rotMat[index] = layerRot*_rotMat[index];
                _pos.col(index) = layerRot*_pos.col(index);;
                _pos.col(index).z() += layerGap;
            }
        }
    }

    _pos.colwise() -= _pos.rowwise().mean();
...
//    std::cout << "eigenSolution.eigenvalues()\n" << eigenSolution.eigenvalues() << "\n";
    _eigenValues = eigenSolution.eigenvalues();
    _eigenVectors = eigenSolution.eigenvectors();
...
VectorXd MagnetEstimator::getEigenValues() const
{
    return _eigenValues;
}
...
void MagnetEstimator::setRotMat(Matrix3d *rotMat)
{
    for(int i = 0; i < _numberOfSensor; i++) _rotMat[i] = rotMat[i];
}

MatrixXd MagnetEstimator::getEigenVectors() const
{
